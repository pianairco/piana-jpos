...
import static ir.piana.dev.jpos.qp.core.http.DefaultRequestHandlerType.INTERNAL_ERROR;
...
public class QPHttpRequestMultiplexorModule
    ...
    protected Map<String, QPHttpHandler> httpHandlerMap =
            new LinkedHashMap<>();
    ...
    protected void configQPModule() throws Exception {
        getPersist().getChildren("qp-http-request")
            ...
            try {
                ...
                QPHttpHandler handler = (QPHttpHandler)getLoader().loadClass(service).newInstance();
    ...
    protected void processRequest(Context context) {
        ...
        try {
            ...
            QPHttpHandler qpHttpHandler = httpHandlerMap.get(request.getRequestURI());
            if (qpHttpHandler == null)
                NOT_FOUND.handle(request, response);
            else {
                if(qpHttpHandler instanceof QPHttpOperator) {
                    invokeHttpOperator((QPHttpOperator) qpHttpHandler, request, response);
                } else {
